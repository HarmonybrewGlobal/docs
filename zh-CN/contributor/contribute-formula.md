...
* **准入规则**：必须符合上游的 [Acceptable-Formulae](https://docs.brew.sh/Acceptable-Formulae) 标准，例如必须是开源软件、必须能从源码构建（不能录入你自己构建好的二进制）等。
...
> **唯一特例**：`ohos-sdk` 是目前核心仓库中唯一无法从流水线构建的特例，因其作为生态基石的特殊地位，我们直接分发官方社区的构建版本。除此之外，不接受任何第二个特例。
